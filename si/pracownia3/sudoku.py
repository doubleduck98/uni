...
def get_3x3(i, j):
    cols = set(get_column(3*i)) \
        | set(get_column(3*i+1)) \
        | set(get_column(3*i+2))
    rows = set(get_row(3*j)) | set(get_row(3*j+1)) | set(get_row(3*j+2))
...
def three_by_three():
    return [all_different(get_3x3(i, j)) for i in range(3) for j in range(3)]
...
def sudoku(assigments):
    ...
    cs = domains(variables) + vertical() + horizontal() + three_by_three()
