...
(* ZADANIE 2 *)
let rec lrepeat f llista =
  let rec lrep = function
      LCons(x, fx), index, 0 -> lrep(fx(), index+1, f (index+1))
    | LCons(x, fx) as ll, index, repeat -> LCons(x, fun() -> lrep(ll, index, repeat-1))
    | LNil, _, _ -> LNil
 in lrep(llista, 0, f 0);;

ltake(10, lrepeat (fun x -> 2) (lfrom 0));;
ltake(7, lrepeat (fun x -> x * 2) (lfrom 0));;
ltake(7, lrepeat (fun x -> x) (toLazyList [1; 2; 3]));;
ltake(7, lrepeat (fun x -> x+1) (toLazyList ['e'; 's'; 'a']));;
